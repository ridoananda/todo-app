...
const title = ref('')
const colors = reactive(['#C4C4C4', '#9B51E0', '#27AE60', '#F2F2F2', '#2D9CDB', '#2F80ED', '#EB5757'])
const emits = defineEmits(['pushItems'])

const saveTask = () => {
    const result = title.value.trim() && title.value
    if (!result || colorActive.value < 0) return
    console.log(colorActive.value);
    let data = { title: title.value, color: colors[colorActive.value] }
    emits('pushItems', data)
    modal.value = false
    title.value = ''
...
<template>
    <div
        class="fixed bottom-4 right-4 w-14 h-14 bg-third rounded-full flex items-center justify-center cursor-pointer add-btn"
        @click="modal = !modal"
    >
        <svg
            width="40"
        ...
        >
            ...
            >
                <div class="w-full relative">
                    <div class="font-bold text-lg text-center font-head">ADD TASK</div>
                    <div class="absolute right-0 top-0 cursor-pointer" @click="modal = false">
                        <svg
                            width="24"
                            height="24"
                            viewBox="0 0 24 24"
